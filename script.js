...
function loadBorrowedBooks() {
    ...
    const userBooks = borrowedBooks.filter(book => book.borrowedBy === currentUser.email);
    if (userBooks.length === 0) {
        borrowedBooksList.innerHTML = '<p>No borrowed books.</p>';
        return;
    }
    //Loop over userBooks, not borrowedBooks
    userBooks.forEach(book => {
        ...
        bookItem.innerHTML = `
            ...
            <div class="book-actions">
                ...
                <button onclick="returnBook('${book.id}')" class="return-btn">Return Book</button>
            </div>
        `;
